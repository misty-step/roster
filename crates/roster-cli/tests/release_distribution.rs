...
fn release_workflow_keeps_version_intelligence_provenance_and_live_replay() {
    ...
    for required in [
        ...
        "git clone --no-tags --no-local . \"$grounding_repo\"",
        "test -z \"$(git -C \"$grounding_repo\" tag --list)\"",
        "release_dist=\"$GITHUB_WORKSPACE/dist\"",
        "--evidence-file \"$release_dist/landmark-run-evidence.json\"",
        "version_decision.commit_count",
        "--changelog-file \"$grounding_repo/CHANGELOG.md\"",
        "--repo-root \"$grounding_repo\"",
